...
function generatePassword() {
  ...
  //final check to force required characters into password in case random selection failed to grab all the required items
  if (upperRequired === true) {
    password[0] = getRandomArray(upperCase)
  }
  if (lowerRequired === true) {
    password[1] = getRandomArray(lowerCase)
  }
  if (numberRequired === true) {
    password[2] = getRandomArray(number)
  }
  if (specialRequired === true) {
    password[3] = getRandomArray(special)
  }
